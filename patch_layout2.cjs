...
const tabsFind = `      <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-4 pb-4">
...
const tabsReplace = `      <div className="flex flex-wrap items-center gap-3 pb-4 mt-2">
          {behaviorConfig.cards.filter((c:any) => ['struggle_pos', 'struggle_neg'].includes(c.id)).map((card: any) => {
              const isActive = activeBehaviorCard === card.id;
              return (
                  <button 
