...
const filePath = path.join('/app/applet/src/components/hizliokuma/Grouping.tsx');
...
export const Grouping = ({ state, actions }: any) => {
    ...
    const activeText = textSource === 'hazir' ? HAZIR_METIN : (customText || 'Lütfen metin giriniz...');
    ...
    }, [isPlaying, currentIndex, blocks.length, timePerGroup]);
                ...
                <div className="p-8 bg-gray-50 rounded-2xl border border-gray-200 text-lg md:text-2xl font-medium leading-loose flex flex-wrap gap-x-2 gap-y-6 min-h-[300px] content-start">
                    {blocks.map((block, i) => {
                        ...
                        if (isNotStarted) {
                            ...
                            blockClass += "bg-purple-200 text-purple-900 font-bold scale-105 shadow-sm";
                        } else if (isPast) {
                            if (mode === 'silinerek') {
                            ...
                            blockClass += "blur-[4px] text-gray-400 bg-gray-200/50 select-none"; // Shadowed/Blurred
                        }
