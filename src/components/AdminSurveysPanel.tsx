...
import { Send, CheckSquare, PlusCircle, Trash2, BarChart2, Sparkles, Calendar, Clipboard, Check, Clock, Edit2, Save, X } from 'lucide-react';
...
export const AdminSurveysPanel = ({ state, actions }: any) => {
  ...
  const [editingExpiryRow, setEditingExpiryRow] = useState<string | null>(null);
                ...
                .map((asgn: any) => {
                  ...
                  return (
                    <tr key={asgn.id} className="hover:bg-gray-50/50 transition-colors">
                      ...
                      <td className="p-4">
                        {editingExpiryRow === asgn.id ? (
                          <div className="flex items-center gap-2">
                            <input
                              type="date"
                              value={editedExpiryDate}
                              onChange={(e) => setEditedExpiryDate(e.target.value)}
                              className="bg-white border border-gray-200 px-2 py-1 rounded-md text-xs font-bold text-gray-700 outline-none focus:border-indigo-500 transition-all"
                            />
                            <button
                              onClick={async () => {
                                if (!editedExpiryDate) return;
                                try {
                                  ...
                                  const targetSchoolId = activeSchoolId || 'default';
                                  const assignmentsCol = targetSchoolId === 'default' ? 'form_assignments' : `form_assignments_${targetSchoolId}`;
                                  updates.push(updateDoc(doc(db, assignmentsCol, asgn.id), { expiryDate: editedExpiryDate }));
                                  
                                  // Update public link if exists
                                  if (asgn.urlSuffix) {
                                    updates.push(updateDoc(doc(db, 'public_surveys', asgn.urlSuffix), { expiryDate: editedExpiryDate }));
                                  }
                                  
                                  await Promise.all(updates);
                                  
                                  setEditingExpiryRow(null);
                                  actions.setAppToast({ message: "Son katılım tarihi güncellendi.", type: 'success' });
                                } catch (e) {
                                  console.error(e);
                                  actions.setAppToast({ message: "Son katılım tarihi güncellenirken hata oluştu.", type: 'error' });
                                }
                              }}
                              className="p-1.5 bg-indigo-50 text-indigo-600 hover:bg-indigo-600 hover:text-white rounded-md transition-colors"
                            >
                              <Save size={14} />
                            </button>
                            <button
                              onClick={() => setEditingExpiryRow(null)}
                              className="p-1.5 bg-gray-50 text-gray-500 hover:bg-gray-200 rounded-md transition-colors"
                            >
                              <X size={14} />
                            </button>
                          </div>
                          ...
                          <div className="flex items-center gap-2 group">
                            ...
                            <button
                              onClick={() => {
                                setEditingExpiryRow(asgn.id);
                                setEditedExpiryDate(asgn.expiryDate || '');
                              }}
          ...
          <div className="lg:col-span-3">
            {selectedResponse ? (
              <div className="bg-white p-8 rounded-3xl shadow-sm border border-gray-100 space-y-6">
                <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between border-b border-gray-100 pb-5 gap-4">
                  ...
                  <div className="flex items-center gap-2">
                    <button 
                      type="button"
                      onClick={async () => {
                        actions.requestConfirm("Bu anket yanıtını silmek istediğinize emin misiniz?", async () => {
                          try {
                            const { doc, deleteDoc } = await import('firebase/firestore');
                            const { db } = await import('../lib/firebase');
                            const guidanceResponsesCol = activeSchoolId === 'default' ? 'guidance_responses' : `guidance_responses_${activeSchoolId}`;
                            await deleteDoc(doc(db, guidanceResponsesCol, selectedResponse.id));
                            setSelectedResponse(null);
                            actions.setAppToast({ message: "Anket yanıtı silindi.", type: 'success' });
                          } catch (e) {
                            console.error(e);
                            actions.setAppToast({ message: "Hata oluştu.", type: 'error' });
                          }
                      ...
                      className="bg-rose-50 border border-rose-100 hover:bg-rose-100 text-rose-600 text-xs px-5 py-2.5 rounded-xl font-bold shadow-sm transition-all cursor-pointer select-none flex items-center gap-2"
                    >
                      <Trash2 size={14}/> Yanıtı Sil
                    </button>
                    <button 
                      type="button"
               ...
               <div className="bg-white p-8 rounded-3xl shadow-sm border border-gray-100 h-full flex flex-col font-sans">
                              ...
                              <button
                                 ...
                                 onClick={async () => {
                                    actions.requestConfirm(
                                       `Bu anket türüne ait tüm (${activeResponses.length}) yanıtları silmek istediğinize emin misiniz? Bu işlem geri alınamaz.`,
                                       async () => {
                                          try {
                                             const { doc, deleteDoc } = await import('firebase/firestore');
                                             const { db } = await import('../lib/firebase');
                                             ...
                                             for (const r of activeResponses) {
                                                await deleteDoc(doc(db, guidanceResponsesCol, r.id));
                                             }
                                             actions.setAppToast({ message: "Anket yanıtları başarıyla temizlendi.", type: 'success' });
                                          } catch (e) {
                                             console.error(e);
                                             actions.setAppToast({ message: "Silme işlemi sırasında hata oluştu.", type: 'error' });
                                          } finally {
                                             setIsLoading(false);
                                          }
                                       }
                                 ...
                                 className="text-gray-400 hover:text-rose-600 font-bold text-[10px] uppercase tracking-wider flex items-center gap-1 transition-colors cursor-pointer"
                              >
                                 <Trash2 size={12} /> Tümünü Sil (Sıfırla)
                              </button>
                           )}
                        </div>
                     </div>
