...
import { ClipboardList, Play, CheckCircle2, XCircle, ArrowRight, Timer, AlertTriangle, Award, BookOpen, Brain, Sparkles, RotateCcw, ChevronRight, Users, Clock, Send } from 'lucide-react';
...
import { getCustomExamsForStudent } from '../../../services/customMitExamService';
import { CustomExamRunner } from './CustomExamRunner';

export const DiagnosticView = ({ grade = 8, state, actions }: { grade?: number; state?: any; actions?: any }) => {
    const studentName = state?.currentUser?.name || state?.currentUser?.username || 'Öğrenci';
    const studentClass = state?.currentUser?.class || `${grade}A`;

    const [activeMode, setActiveMode] = useState<'teacher_exams' | 'meb_standard'>('teacher_exams');
    const [assignedCustomExams, setAssignedCustomExams] = useState<CustomMitExam[]>([]);
    const [selectedCustomExam, setSelectedCustomExam] = useState<CustomMitExam | null>(null);
    const [loadingCustomExams, setLoadingCustomExams] = useState(true);
    ...
    // Öğretmenin atadığı sınavları yükle
    useEffect(() => {
        const fetchExams = async () => {
            setLoadingCustomExams(true);
            const exams = await getCustomExamsForStudent(studentName, studentClass, grade);
            setAssignedCustomExams(exams);
            setLoadingCustomExams(false);
        };
        fetchExams();
    }, [studentName, studentClass, grade]);
    ...
    if (selectedCustomExam) {
        return (
            <CustomExamRunner
                ...
                onBack={() => setSelectedCustomExam(null)}
        ...
        <div className="max-w-4xl mx-auto py-8 px-4 space-y-6">
            ...
            <div className="flex justify-center">
                <div className="inline-flex p-1.5 bg-slate-100 rounded-2xl border border-slate-200">
                    <button
                        type="button"
                        onClick={() => setActiveMode('teacher_exams')}
                        className={`flex items-center gap-2 px-5 py-2.5 rounded-xl text-xs font-black transition-all cursor-pointer ${
                            activeMode === 'teacher_exams'
                                ? 'bg-indigo-600 text-white shadow-md shadow-indigo-200'
                                : 'text-slate-600 hover:text-slate-900'
                        }`}
                    >
                        <Award size={16} />
                        Öğretmenimin Atadığı Sınavlar ({assignedCustomExams.length})
                    </button>
                    <button
                        type="button"
                        onClick={() => setActiveMode('meb_standard')}
                        className={`flex items-center gap-2 px-5 py-2.5 rounded-xl text-xs font-black transition-all cursor-pointer ${
                            activeMode === 'meb_standard'
                                ? 'bg-indigo-600 text-white shadow-md shadow-indigo-200'
                                : 'text-slate-600 hover:text-slate-900'
                        }`}
                    >
                        <ClipboardList size={16} />
                        MEB Hazır Bulunuşluk Sınavı
                    </button>
                </div>
            </div>

            {/* TAB 1: ÖĞRETMENİN ATADIĞI SINAVLAR */}
            {activeMode === 'teacher_exams' && (
                <div className="space-y-6">
                    <div className="text-center space-y-2">
                        <span className="inline-block px-3 py-1 bg-indigo-50 text-indigo-700 rounded-full text-xs font-black uppercase tracking-wider">
                            Öğretmen Özel Tanılama Sınavları
                        </span>
                        <h2 className="text-2xl sm:text-3xl font-black text-slate-900">
                            Öğretmeniniz Tarafından Hazırlanan Sınavlar
                        </h2>
                        <p className="text-slate-500 text-xs sm:text-sm max-w-xl mx-auto">
                            Öğretmeninizin Form 1 (Soru Şablonu) ve Form 2 (Tanılama Matrisi) ile sınıfınıza veya size özel hazırladığı seviye tespit sınavları.
                        </p>
                    </div>
                    ...
                    {loadingCustomExams ? (
                        ...
                        <div className="bg-white p-8 md:p-12 rounded-3xl border border-slate-200 text-center space-y-4 shadow-sm">
                            ...
                            <h3 className="text-base font-black text-slate-800">Şu Anda Bekleyen Özel Sınav Yok</h3>
                            <p className="text-xs text-slate-500 max-w-md mx-auto">
                                Öğretmeniniz henüz sizin sınıfınıza ({studentClass}) özel bir sınav atamadı. Dilerseniz MEB standart hazır bulunuşluk sınavını çözebilirsiniz.
                            </p>
                            <button
                                type="button"
                                onClick={() => setActiveMode('meb_standard')}
                            ...
                            >
                                MEB Seviye Tespit Sınavını Başlat
                            </button>
                        </div>
                    ) : (
                        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                            {assignedCustomExams.map(exam => (
                                <div
                                    key={exam.id}
                                    className="bg-white p-6 rounded-3xl border-2 border-slate-200 hover:border-indigo-300 shadow-xs hover:shadow-md transition-all flex flex-col justify-between space-y-4"
                                >
                                    <div className="space-y-2">
                                        <div className="flex items-center justify-between">
                                            <span className="px-2.5 py-1 rounded-xl bg-indigo-50 text-indigo-700 text-[11px] font-black">
                                                {exam.grade}. Sınıf Seviyesi
                                            </span>
                                    ...
                                    <div className="pt-3 border-t border-slate-100 flex items-center justify-between text-xs text-slate-600">
                                        <span className="flex items-center gap-1.5">
                                            ...
                                            <strong>{exam.questions.length} Soru</strong>
                                        </span>
                                        <span className="flex items-center gap-1.5">
                                            <Clock className="w-4 h-4 text-amber-600" />
                                            <strong>{exam.durationMinutes} Dakika</strong>
                                        </span>
                                    </div>

                                    <button
                                        type="button"
                                        onClick={() => setSelectedCustomExam(exam)}
                                        className="w-full py-3 bg-indigo-600 hover:bg-indigo-700 text-white rounded-2xl font-black text-xs flex items-center justify-center gap-2 shadow-sm transition-all cursor-pointer"
                                    >
                                        <Play className="w-4 h-4 fill-white" /> Sınavı Çözmeye Başla
                                    </button>
                                </div>
                            ))}
                        </div>
            ...
            {activeMode === 'meb_standard' && (
                <div className="text-center space-y-6">
                    ...
                    <div className="space-y-2">
                        ...
                        <h2 className="text-3xl font-black text-slate-900">MEB Matematik Düzey Tanılama</h2>
                    ...
                    <div>
                        <button
                            onClick={handleStart}
                            className="bg-indigo-600 hover:bg-indigo-500 text-white font-black py-4 px-10 rounded-2xl shadow-xl shadow-indigo-200 transition-all active:scale-95 text-base flex items-center gap-2 mx-auto cursor-pointer"
                        >
