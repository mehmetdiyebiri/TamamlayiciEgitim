...
import { CustomExamCreator } from './CustomExamCreator';
...
export const DiagnosticTestManager = ({ state, actions }: { state?: any; actions?: any }) => {
    const [subTab, setSubTab] = useState<'custom_exams' | 'tests' | 'assign' | 'reports'>('custom_exams');
    const [isCreatingCustomExam, setIsCreatingCustomExam] = useState(false);
    const [editingCustomExam, setEditingCustomExam] = useState<CustomMitExam | null>(null);
        ...
        <div className="h-full flex flex-col space-y-6">
            ...
            <div className="flex flex-wrap items-center justify-between gap-4 bg-white p-4 rounded-2xl border border-slate-200 shadow-sm">
                <div className="flex flex-wrap gap-2">
                    <button
                        onClick={() => {
                            setSubTab('custom_exams');
                            setIsCreatingCustomExam(false);
                        }}
                        className={`px-4 py-2 rounded-xl text-xs font-black transition-all flex items-center gap-1.5 ${
                            subTab === 'custom_exams' ? 'bg-indigo-600 text-white shadow-md' : 'text-slate-600 hover:bg-slate-100'
                        }`}
                    >
                        <Sparkles size={14} className={subTab === 'custom_exams' ? 'text-amber-300' : 'text-amber-500'} />
                        Öğretmen Sınavı Hazırla & Dağıt (Form 1 & 2)
                    </button>
                    <button
            ...
            {/* TAB 0: TEACHER-CREATED CUSTOM EXAMS (FORM 1 & FORM 2) */}
            {subTab === 'custom_exams' && (
                <div className="flex-1 space-y-6">
                    {isCreatingCustomExam ? (
                        <CustomExamCreator
                            onBack={() => {
                                setIsCreatingCustomExam(false);
                                setEditingCustomExam(null);
                            }}
                            onSaved={() => {
                                setIsCreatingCustomExam(false);
                                setEditingCustomExam(null);
                                showNotification('Öğretmen seviye tespit sınavı başarıyla oluşturuldu ve dağıtıldı.');
                            }}
                            state={state}
                            showToast={showNotification}
                            initialExam={editingCustomExam}
                        />
                    ) : (
                        <CustomExamList
                            onCreateNew={() => {
                                setEditingCustomExam(null);
                                setIsCreatingCustomExam(true);
                            }}
                            onEditExam={(exam) => {
                                setEditingCustomExam(exam);
                                setIsCreatingCustomExam(true);
                            }}
                            state={state}
                            showToast={showNotification}
