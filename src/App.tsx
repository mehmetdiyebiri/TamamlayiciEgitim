...
import { AdminKelimelikPanel } from './components/AdminKelimelikPanel';
...
import { AdminMakalePanel } from './components/makale/AdminMakalePanel';
import { StudentMakalePanel } from './components/makale/StudentMakalePanel';
import { StudentMitPanel } from './components/mit/StudentMitPanel';
import { PortfolioPanel } from './components/PortfolioPanel';
import { StudentIdeaPanel } from './components/StudentIdeaPanel';
import { StudentDigitalNotebook } from './components/StudentDigitalNotebook';
import { StudentGenelKulturPanel } from './components/StudentGenelKulturPanel';
import { AdminGenelKulturPanel } from './components/AdminGenelKulturPanel';
import { AdminSharePanel } from './components/AdminSharePanel';
import { DBPatches } from './components/DBPatches';
...
import { 
    ...
    BarChart2, Settings, Share2, Menu, X, ChevronRight, Home, User, Lightbulb, Target, Globe, Zap
} from 'lucide-react';
...
export default function App() {
  ...
  const [activeProject, setActiveProject] = useState<'arif' | 'alim' | null>(() => {
    return (sessionStorage.getItem('activeProject') as any) || null;
  });
  const [activeSchoolId, setActiveSchoolId] = useState<string>(() => {
    return sessionStorage.getItem('activeSchoolId') || "";
  });
  ...
  const [activeTab, setActiveTab] = useState(() => {
    ...
    try {
      ...
      return saved || (user?.role === 'student' || user?.role === 'öğrenci' ? 'homework' : 'eval');
    } catch { return saved || 'eval'; }
  });
  useEffect(() => {
  ...
  useEffect(() => {
    if (activeProject) sessionStorage.setItem('activeProject', activeProject);
    else sessionStorage.removeItem('activeProject');
  }, [activeProject]);

  useEffect(() => {
    if (activeSchoolId) sessionStorage.setItem('activeSchoolId', activeSchoolId);
    else sessionStorage.removeItem('activeSchoolId');
  }, [activeSchoolId]);

  useEffect(() => {
    if (activeTab) sessionStorage.setItem('activeTab', activeTab);
    else sessionStorage.removeItem('activeTab');
  }, [activeTab]);
  ...
  // Data State
  const [platformLogo, setPlatformLogo] = useState(() => localStorage.getItem('maarif_platform_logo') || '');
  ...
  useEffect(() => {
    ...
    let unsubscribeBranding: () => void;

    const setupListeners = async () => {
      const { collection, onSnapshot, query, where, doc, getDoc, getDocs } = await import('firebase/firestore');
      const { db } = await import('./lib/firebase');

      // 0. Real-time Platform Branding Listener
      try {
        unsubscribeBranding = onSnapshot(doc(db, 'platform_settings', 'branding'), (docSnap) => {
          ...
          console.warn("Branding listener warning:", err);
        });
      } catch (err) {
  ...
  const [assignmentType, setAssignmentType] = useState('dogruYanlis');
  ...
  // GK State
  const [gkAssignments, setGkAssignments] = useState<any[]>([]);
  const [gkProgress, setGkProgress] = useState<any>({});
  const [gkTab, setGkTab] = useState('new');
  const [selectedGkReportAssignment, setSelectedGkReportAssignment] = useState("");
  const [gkFilterClass, setGkFilterClass] = useState("");
  ...
  const [newUser, setNewUser] = useState<{name: string, username?: string, password?: string, role: string, studentNo?: string, classLevel?: string}>({name: '', username: '', password: '', role: 'teacher'});
  ...
  useEffect(() => {
      ...
      import('firebase/firestore').then(({ collection, doc, onSnapshot }) => {
          import('./lib/firebase').then(({ db }) => {
              ...
              // Listen to gk assignments
              const gkAssignmentsCol = activeSchoolId === 'default' ? 'gk_assignments' : `gk_assignments_${activeSchoolId}`;
              onSnapshot(collection(db, gkAssignmentsCol), (snap) => {
                  const list: any[] = [];
                  snap.forEach(d => list.push({ id: d.id, ...d.data() }));
                  setGkAssignments(list);
              });

              // Listen to gk progress
              const gkProgressCol = activeSchoolId === 'default' ? 'gk_progress' : `gk_progress_${activeSchoolId}`;
              onSnapshot(collection(db, gkProgressCol), (snap) => {
                  const progressMap: any = {};
                  snap.forEach(d => {
                      progressMap[d.id] = d.data();
                  });
                  setGkProgress(progressMap);
              });
                  ...
                  const updateRankings = () => {
                      ...
                      const processData = (dataList: any[]) => {
                          dataList.forEach(({ id, data }) => {
                              ...
                              if (!pMap[studentName]) pMap[studentName] = { activities: [], behaviorLog: [] };
                      ...
                      const pList = Object.keys(pMap).map(k => ({ id: k, fullName: k, activities: pMap[k].activities, behaviorLog: pMap[k].behaviorLog }));
              ...
              // Listen to ideas
              const ideasCol = activeSchoolId === 'default' ? 'ideas' : `ideas_${activeSchoolId}`;
              onSnapshot(collection(db, ideasCol), (snap) => {
                  const list: any[] = [];
                  snap.forEach(d => list.push({ id: d.id, ...d.data() }));
                  setIdeas(list);
              });
  ...
  useEffect(() => {
      ...
      const badgesSourceData = new Map<string, any[]>();
      ...
      const updateDevState = () => {
          ...
          let allBadges: any[] = [];
          Array.from(badgesSourceData.values()).forEach(badges => {
              allBadges = [...allBadges, ...badges];
          });
          const uniqueBadges = Array.from(new Set(allBadges.map(b => typeof b === 'string' ? b : b.id || b)));
          
          setDevCardData({ 
              activities: allActivities.sort((a, b) => new Date(b.date).getTime() - new Date(a.date).getTime()),
              manualBadges: uniqueBadges
          });
      };
              ...
              const listenToDoc = (col: string, id: string, sourceId: string) => {
                  const unsub = onSnapshot(doc(db, col, id), (snap) => {
                      if (snap.exists()) {
                          ...
                          badgesSourceData.set(sourceId, data.manualBadges || []);
                          updateDevState();
                          // Handle Behavior
              ...
              const listenToQuery = (col: string, sourceId: string) => {
                  ...
                  const unsub = onSnapshot(q, (snap) => {
                      ...
                      snap.forEach(d => {
                          ...
                          if (data.manualBadges) mBadges = [...mBadges, ...data.manualBadges];
                      });
                      devSourceData.set(sourceId, devActs);
                      badgesSourceData.set(sourceId, mBadges);
                      behaviorSourceData.set(sourceId, bLogs);
                      updateDevState();
                      updateBehaviorState();
  ...
  const actions = {
          ...
          try {
              ...
              if (!snap.empty) {
                  ...
                  if (user.role === 'student' || user.role === 'öğrenci') {
                    ...
                    const currentSchool = schools.find((s: any) => s.id === loginSchoolId);
                    if (currentSchool?.isSpecialEducation) {
                      setActiveTab('bep');
                    } else {
                      setActiveTab('eval');
                    }
      ...
      handleCreateSchool: async (orgData: any) => {
          ...
          try {
              ...
              const schoolRef = await addDoc(collection(db, 'schools'), {
                  ...
                  isActive: true,
                  activeProjects: orgData.activeProjects || ['arif', 'alim'],
                  isSpecialEducation: orgData.isSpecialEducation || false,
                  createdAt: new Date().toISOString()
              });
      ...
      handleAddActivity: async () => {
          ...
          try {
            ...
            handleToggleBadge: async (badgeId: string | number) => {
          if (!selectedStudent || !activeSchoolId) return;
          try {
      ...
      setGkTab, setSelectedGkReportAssignment, setGkFilterClass,
      ...
      submitIdea: async (text: string) => {
          if (!currentUser || !currentUser.id || !activeSchoolId) return;
          try {
              const { collection, addDoc } = await import('firebase/firestore');
              const { db } = await import('./lib/firebase');
              const ideasCol = activeSchoolId === 'default' ? 'ideas' : `ideas_${activeSchoolId}`;
              await addDoc(collection(db, ideasCol), {
                  studentId: currentUser.id,
                  studentName: currentUser.name,
              ...
              showToast("Fikriniz başarıyla iletildi! Teşekkür ederiz.", "success");
          } catch (error) {
              console.error("Error submitting idea:", error);
              showToast("Fikriniz iletilirken hata oluştu.", "error");
          }
      },
      deleteIdea: async (id: string) => {
          if (!id || !activeSchoolId) return;
          requestConfirm("Bu fikri silmek istediğinize emin misiniz?", async () => {
              try {
                  const { doc, deleteDoc } = await import('firebase/firestore');
                  const { db } = await import('./lib/firebase');
                  const ideasCol = activeSchoolId === 'default' ? 'ideas' : `ideas_${activeSchoolId}`;
                  await deleteDoc(doc(db, ideasCol, id));
                  showToast("Fikir başarıyla silindi.", "success");
              } catch (error) {
      ...
      markIdeaRead: async (id: string) => {
          if (!id || !activeSchoolId) return;
          try {
              const { doc, updateDoc } = await import('firebase/firestore');
              const { db } = await import('./lib/firebase');
              const ideasCol = activeSchoolId === 'default' ? 'ideas' : `ideas_${activeSchoolId}`;
              await updateDoc(doc(db, ideasCol, id), { status: 'read' });
          } catch (error) {
  ...
  const state = {
      ...
      gkAssignments, gkProgress, gkTab, selectedGkReportAssignment, gkFilterClass,
      ...
      <div className="min-h-screen bg-slate-50 flex items-center justify-center p-4 md:p-8 font-sans">
        ...
        {publicSurveyLoading ? (
          <div className="bg-white p-12 rounded-[32px] border border-gray-100 shadow-xl max-w-md w-full text-center space-y-4">
            ...
            <p className="text-xs text-gray-400 font-bold uppercase tracking-wider">MaarifMerkezi.Com - Maarif Asistanınız</p>
          </div>
        ) : publicSurveyError ? (
          ...
          <div className="bg-white p-12 rounded-[32px] border border-gray-100 shadow-xl max-w-lg w-full text-center space-y-6 animate-in zoom-in-95 duration-200">
            ...
            <div className="border-t border-gray-100 pt-6">
              <p className="text-[10px] text-gray-400 font-black uppercase tracking-widest">MaarifMerkezi.Com - Maarif Asistanınız</p>
            </div>
          </div>
        ) : publicSurveyData ? (
          <div className="bg-white p-8 md:p-12 rounded-[40px] border border-gray-100 shadow-2xl max-w-3xl w-full space-y-8 my-8 relative overflow-hidden animate-in zoom-in-95 duration-200">
            ...
            <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between border-b border-gray-100 pb-6 gap-4">
              ...
              <div className="shrink-0 font-extrabold text-right hidden sm:block">
                <div className="text-slate-800 text-lg tracking-wider font-sans font-black">MAARİFMERKEZİ.COM</div>
  ...
  const isSpecialEducation = currentSchool?.isSpecialEducation || false;
  const schoolProjects = isSpecialEducation ? ['alim'] : (currentSchool?.activeProjects || ['arif', 'alim']);

  if (!activeProject && currentUser.role !== 'superadmin' && currentUser.role !== 'provincial_admin' && currentUser.role !== 'district_admin') {
      // Auto select if only one project is active
      if (schoolProjects.length === 1) {
          setActiveProject(schoolProjects[0] as 'arif' | 'alim');
          return null; // Will re-render with activeProject set
      }
          ...
          <div className="min-h-screen bg-gray-50 flex flex-col items-center justify-center p-4">
              <div className="max-w-2xl w-full space-y-12 animate-in fade-in zoom-in-95 duration-500">
                  <div className="text-center space-y-2">
                      <h1 className="text-3xl font-black text-gray-900">Proje Seçimi</h1>
                      <p className="text-gray-500 font-semibold">Lütfen işlem yapmak istediğiniz projeyi seçin.</p>
                  </div>
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
                      {schoolProjects.includes('arif') && (
                          <div onClick={() => { setActiveProject('arif'); setActiveTab(currentUser.role === 'student' ? 'homework' : 'eval'); }} className="bg-white p-10 rounded-[2.5rem] border border-gray-100 shadow-xl shadow-blue-900/5 hover:shadow-2xl hover:border-blue-500 cursor-pointer transition-all duration-300 flex flex-col items-center justify-center gap-6 text-center group">
                              <div className="w-28 h-28 bg-blue-600 text-white rounded-[2rem] flex items-center justify-center group-hover:scale-110 group-hover:-translate-y-2 transition-all duration-300 shadow-lg shadow-blue-600/30 overflow-hidden p-3">
                                  {localStorage.getItem('maarif_platform_logo') ? (
                                      <img src={localStorage.getItem('maarif_platform_logo')!} alt="Logo" className="w-full h-full object-contain filter drop-shadow" />
                                  ) : (
                                      <School size={48} />
                                  )}
                              </div>
                              <div>
                                  <div className="text-3xl font-black text-blue-900 mb-2">MaarifMerkezi</div>
                                  <div className="text-xs font-bold text-gray-400 uppercase tracking-widest">MAARİFMERKEZİ.COM</div>
                              </div>
                          </div>
                      )}
                      {schoolProjects.includes('alim') && (
                          <div onClick={() => { setActiveProject('alim'); setActiveTab('kelimelik'); }} className="bg-white p-10 rounded-[2.5rem] border border-gray-100 shadow-xl shadow-indigo-900/5 hover:shadow-2xl hover:border-indigo-500 cursor-pointer transition-all duration-300 flex flex-col items-center justify-center gap-6 text-center group">
  ...
  const renderContent = () => {
      ...
      if (isStudent) {
          ...
          switch (activeTab) {
              case 'genelkultur': return <StudentGenelKulturPanel state={state} actions={actions} />;
              case 'homework': return <StudentHomePanel state={state} actions={actions} section="homework" />;
              ...
              case 'idea': return <StudentIdeaPanel state={state} actions={actions} />;
              case 'dijitaldefter': return <StudentDigitalNotebook state={state} actions={actions} />;
              case 'profile': return <ProfilePanel state={state} actions={actions} />;
      ...
      if (activeProject === 'alim') {
          switch (activeTab) {
              case 'kelimelik': return <AdminKelimelikPanel state={state} actions={actions} />;
              case 'mit': return <AdminMitPanel state={state} actions={actions} />;
              case 'genelkultur': return <AdminGenelKulturPanel state={state} actions={actions} />;
              case 'homework': return <AdminHomeworkPanel state={state} actions={actions} />;
              case 'makale': return <AdminMakalePanel state={state} actions={actions} />;
              case 'hizliokuma': return <AdminHizliOkumaPanel state={state} actions={actions} />;
              case 'bep': return <BepPanel state={state} actions={actions} />;
              ...
              case 'dijitaldefter': return <StudentDigitalNotebook state={state} actions={actions} />;
              default: return <div className="p-8 text-center"><h2 className="text-2xl font-bold text-gray-800">Seçim Yapın</h2></div>;
          }
      }
  ...
  return (
      <div className={`min-h-screen bg-[#F8FAFC] font-inter text-gray-800 flex flex-col selection:bg-${activeProject === 'alim' ? 'indigo' : 'blue'}-100 selection:text-${activeProject === 'alim' ? 'indigo' : 'blue'}-900`} style={{ '--theme-color': themeColors[activeProject === 'alim' ? 'indigo' : appTheme]?.[600] || '#2563eb', '--theme-bg': themeColors[activeProject === 'alim' ? 'indigo' : appTheme]?.[50] || '#eff6ff' } as React.CSSProperties}>
          {/* Header */}
          <header className="bg-white border-b border-gray-200 px-6 py-4 sticky top-0 z-50 shadow-sm print:hidden">
              <DBPatches activeSchoolId={activeSchoolId} />
              <div className="max-w-[1600px] mx-auto flex items-center justify-between gap-4">
                  {/* Left: School Info */}
                  <div className="flex items-center gap-4">
                      {(() => {
                          const currentLogo = platformLogo || localStorage.getItem('maarif_platform_logo') || schools.find(s => s.id === activeSchoolId)?.logoUrl;
                          return (
                              <div className={`w-12 h-12 rounded-2xl flex items-center justify-center shadow-lg overflow-hidden ${activeProject === 'alim' ? 'shadow-indigo-100' : 'shadow-blue-100'} ${currentLogo ? 'bg-white p-1.5 border border-gray-100' : ''}`} style={!currentLogo ? { backgroundColor: 'var(--theme-color)' } : {}}>
                                  {currentLogo ? (
                      ...
                      <div className="flex flex-col">
                          <h1 className={`font-black text-xl leading-none tracking-tight ${activeProject === 'alim' ? 'text-indigo-900' : 'text-blue-900'}`}>
                              {schools.find(s => s.id === activeSchoolId)?.name || 'Kurum Adı'}
                          </h1>
                          {currentUser.role !== 'student' && (
                            <>
                              <div className="flex items-center gap-2 mt-1">
                                  ...
                                  >
                                      {schools.find(s => s.id === activeSchoolId)?.slogan || (activeProject === 'alim' ? 'Gönül İlim Hikmet Ocağı' : 'Maarif Asistanınız')}
                                  </span>
                              </div>
                              <p className={`text-[10px] font-bold uppercase tracking-widest mt-0.5 ${activeProject === 'alim' ? 'text-indigo-400' : 'text-gray-400'}`}>
                                  {activeProject === 'alim' ? 'ALİM - MAARİFMERKEZİ.COM' : 'MAARİFMERKEZİ.COM - MAARİF ASİSTANINIZ'}
                              </p>
                            </>
                          )}
                      </div>
                  ...
                  <div className="flex items-center gap-4 pl-4 border-l border-gray-100">
                      {schoolProjects.includes('arif') && schoolProjects.includes('alim') && (
                          <button onClick={() => {
                              if (activeProject === 'alim') {
                                  setActiveProject('arif');
                                  setActiveTab(currentUser.role === 'student' ? 'homework' : 'eval');
                              } else {
                                  setActiveProject('alim');
                                  setActiveTab('kelimelik');
                              }
                          }} className={`px-4 py-2 rounded-xl font-bold text-xs transition-colors ${activeProject === 'alim' ? 'bg-blue-600 text-white hover:bg-blue-700' : 'bg-indigo-600 text-white hover:bg-indigo-700'}`}>
                              {activeProject === 'alim' ? "MaarifMerkezi'ne Geç" : "ALİM'e Geç"}
                          </button>
                      )}
                  ...
                  {currentUser.role !== 'student' ? (
                    <>
                      {activeProject === 'alim' ? (
                          isSpecialEducation ? (
                              <>
                                  <button onClick={() => setActiveTab('bep')} className={`flex items-center gap-2.5 py-4 text-sm font-bold transition-all whitespace-nowrap border-b-2 ${activeTab === 'bep' ? 'border-indigo-600 text-indigo-700' : 'border-transparent text-gray-400 hover:text-gray-800 hover:border-gray-300'}`}>
                                      <Activity size={18} className={activeTab === 'bep' ? 'text-indigo-600' : 'text-gray-400'}/> Bep Modülü
                                  </button>
                                  {currentUser.role === 'admin' && (
                                      <button onClick={() => setActiveTab('teacherperf')} className={`flex items-center gap-2.5 py-4 text-sm font-bold transition-all whitespace-nowrap border-b-2 ${activeTab === 'teacherperf' ? 'border-indigo-600 text-indigo-700' : 'border-transparent text-gray-400 hover:text-gray-800 hover:border-gray-300'}`}>
                                          <Activity size={18} className={activeTab === 'teacherperf' ? 'text-indigo-600' : 'text-gray-400'}/> Öğretmen Performans
                                      </button>
                                  )}
                                  <button onClick={() => setActiveTab('profile')} className={`flex items-center gap-2.5 py-4 text-sm font-bold transition-all whitespace-nowrap border-b-2 ${activeTab === 'profile' ? 'border-indigo-600 text-indigo-700' : 'border-transparent text-gray-400 hover:text-gray-800 hover:border-gray-300'}`}>
                                      <User size={18} className={activeTab === 'profile' ? 'text-indigo-600' : 'text-gray-400'}/> Profilim
                                  </button>
                                  {currentUser.role === 'admin' && (
                                      <>
                                          <button onClick={() => setActiveTab('admin')} className={`flex items-center gap-2.5 py-4 text-sm font-bold transition-all whitespace-nowrap border-b-2 ${activeTab === 'admin' ? 'border-indigo-600 text-indigo-700' : 'border-transparent text-gray-400 hover:text-gray-800 hover:border-gray-300'}`}>
                                              <Settings size={18} className={activeTab === 'admin' ? 'text-indigo-600' : 'text-gray-400'}/> Yönetim Paneli
                                          </button>
                                      </>
                                  )}
                              </>
                          ...
                          <>
                              <button onClick={() => setActiveTab('kelimelik')} className={`flex items-center gap-2.5 py-4 text-sm font-bold transition-all whitespace-nowrap border-b-2 ${activeTab === 'kelimelik' ? 'border-indigo-600 text-indigo-700' : 'border-transparent text-gray-400 hover:text-gray-800 hover:border-gray-300'}`}>
                                  <BookOpen size={18} className={activeTab === 'kelimelik' ? 'text-indigo-600' : 'text-gray-400'}/> Kelimelik 
                              ...
                              <button onClick={() => setActiveTab('mit')} className={`flex items-center gap-2.5 py-4 text-sm font-bold transition-all whitespace-nowrap border-b-2 ${activeTab === 'mit' ? 'border-indigo-600 text-indigo-700' : 'border-transparent text-gray-400 hover:text-gray-800 hover:border-gray-300'}`}>
                                  <Target size={18} className={activeTab === 'mit' ? 'text-indigo-600' : 'text-gray-400'}/> MİT
                              </button>
                              <button onClick={() => setActiveTab('genelkultur')} className={`flex items-center gap-2.5 py-4 text-sm font-bold transition-all whitespace-nowrap border-b-2 ${activeTab === 'genelkultur' ? 'border-indigo-600 text-indigo-700' : 'border-transparent text-gray-400 hover:text-gray-800 hover:border-gray-300'}`}>
                                  <Globe size={18} className={activeTab === 'genelkultur' ? 'text-indigo-600' : 'text-gray-400'}/> Genel Kültür
                              </button>
                              <button onClick={() => setActiveTab('homework')} className={`flex items-center gap-2.5 py-4 text-sm font-bold transition-all whitespace-nowrap border-b-2 ${activeTab === 'homework' ? 'border-indigo-600 text-indigo-700' : 'border-transparent text-gray-400 hover:text-gray-800 hover:border-gray-300'}`}>
                                  <BookOpen size={18} className={activeTab === 'homework' ? 'text-indigo-600' : 'text-gray-400'}/> Ödev
                              </button>
                              <button onClick={() => setActiveTab('makale')} className={`flex items-center gap-2.5 py-4 text-sm font-bold transition-all whitespace-nowrap border-b-2 ${activeTab === 'makale' ? 'border-indigo-600 text-indigo-700' : 'border-transparent text-gray-400 hover:text-gray-800 hover:border-gray-300'}`}>
                                  <FileText size={18} className={activeTab === 'makale' ? 'text-indigo-600' : 'text-gray-400'}/> Makale Okulu
                              </button>
                              <button onClick={() => setActiveTab('hizliokuma')} className={`flex items-center gap-2.5 py-4 text-sm font-bold transition-all whitespace-nowrap border-b-2 ${activeTab === 'hizliokuma' ? 'border-indigo-600 text-indigo-700' : 'border-transparent text-gray-400 hover:text-gray-800 hover:border-gray-300'}`}>
                                  <Zap size={18} className={activeTab === 'hizliokuma' ? 'text-indigo-600' : 'text-gray-400'}/> Hızlı Okuma
                              </button>
                              <button onClick={() => setActiveTab('bep')} className={`flex items-center gap-2.5 py-4 text-sm font-bold transition-all whitespace-nowrap border-b-2 ${activeTab === 'bep' ? 'border-indigo-600 text-indigo-700' : 'border-transparent text-gray-400 hover:text-gray-800 hover:border-gray-300'}`}>
                                  <Activity size={18} className={activeTab === 'bep' ? 'text-indigo-600' : 'text-gray-400'}/> Bep Modülü
                              </button>
                          ...
                          <>
                              ...
                              <button onClick={() => setActiveTab('dijitaldefter')} className={`flex items-center gap-2.5 py-4 text-sm font-bold transition-all whitespace-nowrap border-b-2 ${activeTab === 'dijitaldefter' ? 'border-emerald-600 text-emerald-700' : 'border-transparent text-gray-400 hover:text-gray-800 hover:border-gray-300'}`}>
                                  <BookOpen size={18} className={activeTab === 'dijitaldefter' ? 'text-emerald-600' : 'text-gray-400'}/> Dijital Defter
                              </button>
                    ...
                    <>
                      {activeProject === 'alim' ? (
                          ...
                          <>
                              ...
                              <button onClick={() => setActiveTab('homework')} className={`flex items-center gap-2.5 py-4 text-sm font-bold transition-all whitespace-nowrap border-b-2 ${activeTab === 'homework' ? 'border-indigo-600 text-indigo-700' : 'border-transparent text-gray-400 hover:text-gray-800 hover:border-gray-300'}`}>
                                  <BookOpen size={18} className={activeTab === 'homework' ? 'text-indigo-600' : 'text-gray-400'}/> Ödevlerim
                              </button>
                              <button onClick={() => setActiveTab('makale')} className={`flex items-center gap-2.5 py-4 text-sm font-bold transition-all whitespace-nowrap border-b-2 ${activeTab === 'makale' ? 'border-indigo-600 text-indigo-700' : 'border-transparent text-gray-400 hover:text-gray-800 hover:border-gray-300'}`}>
                                  <FileText size={18} className={activeTab === 'makale' ? 'text-indigo-600' : 'text-gray-400'}/> Makale Okulu
                              </button>
                              ...
                              <button onClick={() => setActiveTab('profile')} className={`flex items-center gap-2.5 py-4 text-sm font-bold transition-all whitespace-nowrap border-b-2 ${activeTab === 'profile' ? 'border-indigo-600 text-indigo-700' : 'border-transparent text-gray-400 hover:text-gray-800 hover:border-gray-300'}`}>
                                  <User size={18} className={activeTab === 'profile' ? 'text-indigo-600' : 'text-gray-400'}/> Profil Bilgileri
                              </button>
                          </>
                          )
                      ) : (
                          <>
                              <button onClick={() => setActiveTab('genelkultur')} className={`flex items-center gap-2.5 py-4 text-sm font-bold transition-all whitespace-nowrap border-b-2 ${activeTab === 'genelkultur' ? 'border-blue-600 text-blue-700' : 'border-transparent text-gray-400 hover:text-gray-800 hover:border-gray-300'}`}>
                                  <Globe size={18} className={activeTab === 'genelkultur' ? 'text-blue-600' : 'text-gray-400'}/> Genel Kültür
                              </button>
                              <button onClick={() => setActiveTab('homework')} className={`flex items-center gap-2.5 py-4 text-sm font-bold transition-all whitespace-nowrap border-b-2 ${activeTab === 'homework' ? 'border-blue-600 text-blue-700' : 'border-transparent text-gray-400 hover:text-gray-800 hover:border-gray-300'}`}>
                                  <BookOpen size={18} className={activeTab === 'homework' ? 'text-blue-600' : 'text-gray-400'}/> Ödevlerim
                                  {assignments.filter((a: any) => {
                              ...
                              <button onClick={() => setActiveTab('idea')} className={`flex items-center gap-2.5 py-4 text-sm font-bold transition-all whitespace-nowrap border-b-2 ${activeTab === 'idea' ? 'border-rose-600 text-rose-700' : 'border-transparent text-gray-400 hover:text-gray-800 hover:border-gray-300'}`}>
                                  <Lightbulb size={18} className={activeTab === 'idea' ? 'text-rose-600' : 'text-gray-400'}/> Bir Fikrim Var
                              </button>
                              <button onClick={() => setActiveTab('dijitaldefter')} className={`flex items-center gap-2.5 py-4 text-sm font-bold transition-all whitespace-nowrap border-b-2 ${activeTab === 'dijitaldefter' ? 'border-emerald-600 text-emerald-700' : 'border-transparent text-gray-400 hover:text-gray-800 hover:border-gray-300'}`}>
                                  <BookOpen size={18} className={activeTab === 'dijitaldefter' ? 'text-emerald-600' : 'text-gray-400'}/> Dijital Defter
                              </button>
